...
def get_mean_head_activations(dataset, model, model_config, tokenizer, n_icl_examples = 10, N_TRIALS = 100, shuffle_labels=False, prefixes=None, separators=None, filter_set=None):
    ...
    for n in tqdm(range(N_TRIALS), desc='Trial on extraction'):
        ...
        #print('stack_initial shape:', stack_initial.shape)
        stack_filtered = stack_initial[:,:,list(idx_map.keys())]
        #print(len(list(idx_map.keys())))
        #print('stack_filtered shape:', stack_filtered.shape)
        ...
        #print('activations shape:', activation_storage.shape)
        #print('stack_initial shape:', stack_initial.shape)
        #print('stack_filtered shape:', stack_filtered.shape)

...
import argparse
from Support_Utils.Prompt_Template import *
...
'''
def template_setting(data_cases, model, tokenizer, stop_words, stop_num, batch_size, max_length):
...
'''
def processing(template_type, subset_root, subset_root_save, file_type, test_file, head_type, model, tokenizer, topk, lang_target, batch_size, max_length, operation):
    data = BR(os.path.join(subset_root, test_file)).data_reader['json']()
    if len(data) == 0:
        print('The data is empty for file!'.format(test_file))
        return 0
    sorted_data = sorted(data, key=lambda x: x['source'], reverse=True)
    setting_shot = template_type.split('_')[0]
    setting_template = template_type.split('_')[1]

    if 'zero' == setting_shot:
        sorted_cases = [Prompt_Template_Sum(setting_shot).templates[setting_template](item['src_lang'], item['trg_lang'], item['source']) for item in sorted_data]
    if 'one' == setting_shot:
        sorted_cases = [Prompt_Template_Sum(setting_shot).templates[setting_template](item['example_src'], item['example_trg'], item['src_lang'], item['trg_lang'], item['source']) for item in sorted_data]
    ...
    if os.path.exists(os.path.join(subset_root_save, f'{head_type}_top-{topk}-head_{operation}_gene_{file_type}' + test_file)):
        ...
        if 'one_shot' in test_file:
            stop_words = ['\n']
            stop_num = 4
            genes = template_setting(sorted_cases, model, tokenizer,
...
def template_check(head_type, template_type, operation='none', top_k=0, shot_setting = ['one', 'zero']):
    if 'one' in shot_setting:
        template_type_one = 'one_' + template_type
        eval_files = []
        ...
        for test_file in one_shot_files:
            ...
            set_result = processing(template_type_one, one_shot_root, subset_root_save, file_type, test_file, head_type, model, tokenizer, top_k, lang_target, batch_size, max_length, operation)
    ...
    if 'zero' in shot_setting:
        # zero-shot
        eval_files = []
        overall_set_eval = list()
        template_type_zero = 'zero_' + template_type
        file_type = 'Test'
        ...
        for test_file in zero_shot_files:
            ...
            set_result = processing(template_type_zero,  zero_shot_root, subset_root_save, file_type, test_file, head_type, model, tokenizer, top_k, lang_target, batch_size, max_length, operation)
            overall_set_eval.append(set_result)
            eval_files.append(test_file)
            # storage the evaluation results
...
if __name__ == '__main__':
    # generation with different prompt templates
    ...
    parser.add_argument('--template_type', help='template type', type=str, required=True, default=None)
    parser.add_argument('--file_type', help='file type for the testing or validation', type=str, required=True, default='Test')
    parser.add_argument('--shot_setting', help='shot setting for the templates', type=str, required=True, default='Test')
    ...
    shot_setting = [str(item) for item in args.shot_setting.split(',')]

    #temp1
    head_type = args.heads_type
    template_type = args.template_type
    template_check(head_type, template_type,  shot_setting=shot_setting)

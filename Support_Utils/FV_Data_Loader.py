...
def create_fewshot_primer(prompt_data) -> str:
    ...
    """
    prompt = ''
    prompt += prompt_data['prefixes']['instructions'] + prompt_data['instructions'] + prompt_data['separators'][
        'instructions']
...
def get_prompt_parts_and_labels(prompt_data, query_sentence=None):
    ...
    assemble_icl_example = lambda example, prompt_data: [prompt_data['prefixes']['input'], example['input'],
                                                         prompt_data['separators']['input'],
                                                         prompt_data['prefixes']['output'], example['output'],
    ...
    prompt_instructions = [prompt_data['prefixes']['instructions'], prompt_data['instructions'],
    ...
    prompt_icl_examples_labels = [
        ['structural_token', f'demonstration_{i + 1}_token', 'separator_token', 'structural_token',
         f'demonstration_{i + 1}_label_token', 'separator_token'] for i in range(n_examples)]
    prompt_icl_query_labels = [
        ['query_structural_token', 'query_demonstration_token', 'query_separator_token', 'query_structural_token']]
...
'''replace the duplicated labels with the ground truth labels'''


def compute_duplicated_labels_align(token_labels, gt_labels):
    index_map = dict()  # token string index to ground truth index based on the label alignment

    # make the alignment algorithm
    j = 0
    for i in token_labels:
        index_ = i[0]
        real_label = i[2]

        current_gt_label = gt_labels[j][1]
        if real_label == current_gt_label:
            index_map[index_] = gt_labels[j][0]
            j += 1
        else:
            pass
    # count the duplicated labels
    list_duplicated = list(index_map.keys())
    max_length = len(gt_labels)

    dup_maps = dict()
    for index_, item in enumerate(list_duplicated):
        if index_ + 1 < max_length:
            if list_duplicated[index_ + 1] - item == 1:
                pass
            else:
                dup_maps[index_map[item]] = (item, list_duplicated[index_ + 1] - 1)
    return index_map, dup_maps
...
def word_pairs_to_prompt_data(word_pairs: dict,
    ...
    if shuffle_labels:
        randomized_pairs = [np.random.permutation(x).tolist() if i == 1 else x for (i, x) in
                            enumerate(list(word_pairs.values()))]  # shuffle labels only
        if prepend_space:
            prompt_data['examples'] = [{'input': ' ' + w1, 'output': ' ' + w2} for (w1, w2) in
                                       list(zip(*randomized_pairs))]
            prompt_data['query_target'] = {k: ' ' + v for k, v in
                                           query_target_pair.items()} if query_target_pair is not None else None
        else:
            prompt_data['examples'] = [{'input': w1, 'output': w2} for (w1, w2) in list(zip(*randomized_pairs))]
    else:
        if prepend_space:
            prompt_data['examples'] = [{'input': ' ' + w1, 'output': ' ' + str(w2)} for (w1, w2) in
                                       list(zip(*word_pairs.values()))]
            prompt_data['query_target'] = {k: ' ' + str(v) for k, v in
                                           query_target_pair.items()} if query_target_pair is not None else None
        else:
...
def split_icl_dataset(dataset, train_size=None, test_size=0.3, seed=42) -> Dict[str, ICLDataset]:
    ...
    train, valid = train_test_split(dataset.raw_data, train_size=train_size, test_size=test_size, random_state=seed)
    #test, valid = train_test_split(valid, train_size=train_size, test_size=test_size, random_state=seed)
...
def load_dataset(task_name,
    ...
    if isinstance(task_name, str):
        data_folders = ['FV_Data_Adapt']
        assert test_size <= 1.0

        path = Path(root_data_dir)
        d_group_map = [(dataset_type, os.path.exists(os.path.join(root_data_dir, dataset_type, task_name + '.json'))) for
                       dataset_type in data_folders]

        d_group = list(filter(lambda x: x[1], d_group_map))

        assert len(d_group) != 0 and len(
            d_group) == 1, f"Error! 'task_name'={task_name}.json must be uniquely contained in one of these directories:{data_folders}. Please check the root_data_dir"
